...
#         self.add_output('hub_system_I', desc='mass moments of Inertia of hub [Ixx, Iyy, Izz, Ixy, Ixz, Iyz] around its center of mass in yaw-aligned c.s.', shape=3)
...
#         L_rb = 0 #inputs['L_rb']
#         shaft_angle = 0 #inputs['shaft_angle']
#         MB1_location = [0.0, 0.0, 0.0] #inputs['MB1_location']
...
class Hub_drive(ExplicitComponent):
    ''' Hub class    
    ...
    def setup(self):
        ...
        # outputs
        self.add_output('diameter', units='m', desc='hub diameter', val=0.0)
        self.add_output('thickness', units='m',desc='hub thickness', val=0.0)
    ...
    def compute(self, inputs, outputs):
        ...
        if blade_root_diameter > 0.0: #added 8/6/14 to allow analysis of hubs for unknown blade roots.
            blade_root_diameter = blade_root_diameter
        else:
            blade_root_diameter = 2.659*machine_rating**.3254
        ...
        outputs['thickness']=castThickness   
...
class HubSE(Group):
    def setup(self):
        ...
        # parameters
        i.add_output('blade_number', desc='number of turbine blades', val=3)
        
        
        
        self.add_subsystem('dof', i)        
        self.add_subsystem('hub', Hub_drive(), promotes_outputs=[('mass', 'hub_mass'), 'thickness', 'diameter'])
        self.add_subsystem('pitchSystem', PitchSystem_drive(), promotes_outputs=[('mass', 'pitch_mass')])
        self.add_subsystem('spinner', Spinner_drive(), promotes_outputs=[('mass', 'spinner_mass')])
#         self.add_subsystem('adder', Hub_System_Adder_drive(), promotes_outputs=['hub_system_cm', 'hub_system_I', 'hub_system_mass'])
...
if __name__ == "__main__":
    ...
    print prob['hub_mass']  # 31644.47
    print prob['pitch_mass'] # 17003.98
    print prob['spinner_mass'] # 1810.50
    print prob['diameter'] 
