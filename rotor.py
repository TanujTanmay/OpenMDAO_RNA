...
from rotor_design import RotorDesignBetz, RotorDesignScaling
...
################################  WORKFLOWS #################################
#############################################################################
class RotorAnalytical(Group):
    def setup(self):
        
        self.add_subsystem('design', RotorDesignBetz(), \
                           promotes_inputs=['*'], \
                           promotes_outputs=['*'])
        
        self.add_subsystem('aero', AerodynamicsAeroDyn(), \
                           promotes_inputs=['*'], \
                           promotes_outputs=['*'])
        
        self.add_subsystem('pc', PowerCurve(), \
                           promotes_inputs=['*'], \
                           promotes_outputs=['*'])                    
        
        self.add_subsystem('struc', RotorMechanicsAnalytical(), \
                           promotes_inputs=['*'], \
                           promotes_outputs=['*'])        
...
class RotorScaled(Group):
    def setup(self):
        
        self.add_subsystem('design', RotorDesignScaling(), \
                           promotes_inputs=['*'], \
                           promotes_outputs=['*'])
...
class RotorTest(Group):
    def setup(self):
        ...
        self.add_subsystem('rotor', RotorScaled(), promotes_inputs=['*'])
...
if __name__ == "__main__":
    ...
    prob.setup()
    view_model(prob, outfile='N2/rotor.html')
