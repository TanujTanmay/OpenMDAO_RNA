...
class RotorDesignBetz(Group):
    ...
    '''
    def setup(self):             
        self.add_subsystem('aero', AerodynamicDesignBetz(), \
                           promotes_inputs = ['*'], \
                           promotes_outputs=['*']
                           )
        
        self.add_subsystem('struc', StructuralDesignConstantRadius(), \
                           promotes_inputs = ['*'], \
                           promotes_outputs=['*'])



class RotorDesignScaling(Group):
    '''
        this setup is used to design the rotor when the following holds true:
        - aerodynamic profile is scaled from the reference turbine
        - structural profile is also scaled from the reference turbine
    '''
    def setup(self):             
        self.add_subsystem('aero', AerodynamicDesignScaling(), \
                           promotes_inputs = ['*'], \
                           promotes_outputs=['*']
                           )
        
        self.add_subsystem('struc', StructuralDesignVariableRadius(), \
                           promotes_inputs = ['*'], \
                           promotes_outputs=['*'])
...
class RotorDesignTest(Group):
    def setup(self):
        ...
        self.add_subsystem('design', RotorDesignScaling(), promotes_inputs=['*'])
