...
from fixed_parameters import num_bins, num_nodes, rho_air, spanwise_params, z0, h_ref, wind_shear, degree_blade_param, airfoils_db
...
class BladeProfiling(ExplicitComponent):
    def setup(self):
        # variables
...
class AeroAdder(Group):
    def setup(self):
        ...
        self.add_subsystem('profile', BladeProfiling(), \
                           promotes_inputs = ['blade_number', 'rotor_diameter', 'rotor_speed', 'span_r', 'span_airfoil', \
                                              'chord_params', 'twist_params', 'adjust_pitch'], \
                           promotes_outputs =['pitch'])
             
        self.add_subsystem('annulus', AnnulusAero(), \
                           promotes_inputs = ['blade_number', 'rotor_diameter', 'rotor_speed', 'hub_radius', \
                                              'span_r', 'span_dr', 'span_airfoil'])
        ...
        self.connect('profile.span_chord', 'annulus.span_chord') 
        self.connect('profile.span_twist', 'annulus.span_twist')  
        ...
        self.connect('annulus.span_fx', 'hub.span_fx')  
        self.connect('annulus.span_fy', 'hub.span_fy') 
...
class RotorAeroGroupTest(Group):
    def setup(self):
        ...
        i.add_output('span_dr', units='m', desc='list of blade annulus thickness', shape=num_nodes)
...
if __name__ == "__main__":
    ...
    prob['dof.span_r'] = [11.0, 15.0, 25.0, 35.0, 45.0]
    ...
    print prob['aero.swept_area'] 
    print prob['aero.rotor_tsr'] 
    print prob['aero.pitch'] 
    print prob['aero.rotor_power'] 
    print prob['aero.rotor_thrust'] 
    print prob['aero.rotor_cp'] 
    print prob['aero.rotor_cq'] 
    print prob['aero.rotor_ct'] 
