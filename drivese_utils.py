...
def setup_Bedplate(L_rb, rotor_diameter, \
                    transformer_mass, transformer_location, machine_rating, generator_location, \
    ...
    if L_rb>0:
        ...
        [L_rb] = get_L_rb(rotor_diameter,False)
    
    #component weights and locations
    if transformer_mass>0: #only if uptower transformer
        transLoc = transformer_location.item()
        convMass = 0.3*transformer_mass
    else:
        transLoc = 0
        convMass = (2.4445*(machine_rating) + 1599.0)*0.3 #(transformer mass * .3)
    
    convLoc = generator_location * 2.0
    #TODO: removed  since this are from connections but not sure if that disruptes upstream usage
    mb1_location = abs(mb1_location) #abs(gbx_length/2.0) + abs(lss_length)
    mb2_location = abs(mb2_location) #abs(gbx_length/2.0)
    lss_location= abs(lss_location)
    
    if transLoc > 0:
      rearTotalLength = transLoc*1.1
    else:
      rearTotalLength = generator_location*4.237/2.886 -tower_top_diameter/2.0 #scaled off of GE1.5
    
    frontTotalLength = mb1_location + FW_mb1/2.
    
    #rotor weights and loads
    rotorLoc = mb1_location + L_rb
    rotorFz=abs(rotor_force_z)
    rotorMy=abs(rotor_bending_moment_y)
    
    #If user does not know important moment, crude approx
    if rotor_mass > 0 and rotorMy == 0: 
        rotorMy=get_My(rotor_mass,L_rb)
    
    if rotorFz == 0 and rotor_mass >0:
        rotorFz = rotor_mass*g
...
def characterize_Bedplate_Rear(tf, tw, h0, b0, density, g, E, rearTotalLength, \
    ...
    hssTipDefl = midDeflection(rearTotalLength,hss_location,hss_mass*g/2,E,I_b)
    genTipDefl = midDeflection(rearTotalLength,generator_location,generator_mass*g/2,E,I_b)
    convTipDefl = midDeflection(rearTotalLength,convLoc,convMass*g/2,E,I_b)
    transTipDefl = midDeflection(rearTotalLength,transLoc,transformer_mass*g/2,E,I_b)
    gbxTipDefl = midDeflection(rearTotalLength,gbx_location,gbx_mass*g/2,E,I_b)
    selfTipDefl = distDeflection(rearTotalLength,w*g,E,I_b)
    
    totalTipDefl = hssTipDefl + genTipDefl + convTipDefl + transTipDefl +  selfTipDefl + gbxTipDefl
    
    #root stress
    totalBendingMoment=(hss_location*hss_mass + generator_location*generator_mass + convLoc*convMass + transLoc*transformer_mass + w*rearTotalLength**2/2.0)*g
    rootStress = totalBendingMoment*h0/(2.*I_b)
...
def setup_Bedplate_Front(gbx_location, gbx_mass, frontTotalLength, defl_denom):
    if gbx_location < 0:
        gbx_location = abs(gbx_location)
        gbx_mass = gbx_mass
    else: 
        gbx_location = 0
        gbx_mass = 0
...
def characterize_Bedplate_Front(tf, tw, h0, b0, castDensity, g, E, frontTotalLength, \
    ...
    #Tip Deflection for load not at end
    gbxTipDefl = midDeflection(frontTotalLength,gbx_location,gbx_mass*g/2.0,E,I_b)
    mb1TipDefl = midDeflection(frontTotalLength,mb1_location,mb1_mass*g/2.0,E,I_b)
    mb2TipDefl = midDeflection(frontTotalLength,mb2_location,mb2_mass*g/2.0,E,I_b)
    lssTipDefl = midDeflection(frontTotalLength,lss_location,lss_mass*g/2.0,E,I_b)
    rotorTipDefl = midDeflection(frontTotalLength,rotorLoc,rotor_mass*g/2.0,E,I_b)
    rotorFzTipDefl = midDeflection(frontTotalLength,rotorLoc,rotorFz/2.0,E,I_b)
    selfTipDefl = distDeflection(frontTotalLength,w*g,E,I_b)
    rotorMyTipDefl = rotorMy/2.0*frontTotalLength**2/(2.0*E*I_b)
    
    totalTipDefl = mb1TipDefl + mb2TipDefl + lssTipDefl  + rotorTipDefl + selfTipDefl +\
      rotorMyTipDefl + rotorFzTipDefl + gbxTipDefl
    
    #root stress
    totalBendingMoment=(mb1_location*mb1_mass/2.0 + mb2_location*mb2_mass/2.0 + lss_location*\
      lss_mass/2.0 + w*frontTotalLength**2/2.0 + rotorLoc*rotor_mass/2.0)*g + rotorLoc*\
      rotorFz/2.0 +rotorMy/2.0
    rootStress = totalBendingMoment*h0/2/I_b
...
def size_Bedplate(b0, rotor_diameter, tower_top_diameter, frontHeight, rearHeight, \
                  frontTotalLength, rearTotalLength, totalCastMass, totalSteelMass):
    #frame multiplier for front support
    support_multiplier = 1.1+5e13*rotor_diameter**(-8) # based on solidworks estimates for GRC and GE bedplates. extraneous mass percentage decreases for larger machines
    ...
    cm = np.array([0.0,0.0,0.0])
    cm[0] = (totalSteelMass*rearTotalLength/2 - totalCastMass*frontTotalLength/2)/(mass) #previously 0.
    cm[1] = 0.0
    cm[2] = -height/2.
    cm = cm
    ...
    I[0]  = mass * (width ** 2 + depth ** 2) / 8
    I[1]  = mass * (depth ** 2 + width ** 2 + (4/3) * length ** 2) / 16
    I[2]  = I[1]
    I = I
...
class blade_moment_transform(ExplicitComponent): 
    ...
    def setup(self):
        ...
        self.add_input('azimuth_angle', units='rad',    desc='azimuth angles for each blade', val=np.array([0,2*pi/3,4*pi/3]))
        self.add_input('pitch_angle',   units='rad',    desc='pitch angle at each blade, assumed same', val=0.0)
        self.add_input('cone_angle',    units='rad',    desc='cone angle at each blade, assumed same', val=0.0)
        self.add_input('b1',            units='N*m',    desc='moments in x,y,z directions along local blade coordinate system', shape=3)
        self.add_input('b2',            units='N*m',    desc='moments in x,y,z directions along local blade coordinate system', shape=3)
        self.add_input('b3',            units='N*m',    desc='moments in x,y,z directions along local blade coordinate system', shape=3)
    
        # returns
        self.add_output('Mx', units='N*m', desc='rotor moment in x-direction')
        self.add_output('My', units='N*m', desc='rotor moment in y-direction')
        self.add_output('Mz', units='N*m', desc='rotor moment in z-direction')
    ...
    def compute(self, inputs, outputs):
        ...
        azimuth_angle = inputs['azimuth_angle']
        pitch_angle = inputs['pitch_angle']
        cone_angle = inputs['cone_angle']
        b1 = inputs['b1']
        b2 = inputs['b2']
        b3 = inputs['b3']
        ...
        outputs['Mx'] = b1Mx+b2Mx+b3Mx
        outputs['My'] = b1My+b2My+b3My
        outputs['Mz'] = b1Mz+b2Mz+b3Mz
...
class blade_force_transform(ExplicitComponent): 
    ...
    def setup(self):
        ...
        self.add_input('pitch_angle',   units='rad',    desc='pitch angle at each blade, assumed same', val=0.0)
        self.add_input('cone_angle',    units='rad',    desc='cone angle at each blade, assumed same', val=0.0)
        self.add_input('b1',            units='N',      desc='forces in x,y,z directions along local blade coordinate system', shape=3)
        self.add_input('b2',            units='N',      desc='forces in x,y,z directions along local blade coordinate system', shape=3)
        self.add_input('b3',            units='N',      desc='forces in x,y,z directions along local blade coordinate system', shape=3)
    
        # returns
        self.add_output('Fx', units='N', desc='rotor force in x-direction')
        self.add_output('Fy', units='N', desc='rotor force in y-direction')
        self.add_output('Fz', units='N', desc='rotor force in z-direction')
    ...
    def compute(self, inputs, outputs):
        ...
        [b1Fx,b1Fy,b1Fz] = trans(pitch_angle,cone_angle,azimuth_angle[0],b1[0],b1[1],b1[2])
        [b2Fx,b2Fy,b2Fz] = trans(pitch_angle,cone_angle,azimuth_angle[1],b2[0],b2[1],b2[2])
        [b3Fx,b3Fy,b3Fz] = trans(pitch_angle,cone_angle,azimuth_angle[2],b3[0],b3[1],b3[2])

        outputs['Fx'] = b1Fx+b2Fx+b3Fx
        outputs['Fy'] = b1Fy+b2Fy+b3Fy
        outputs['Fz'] = b1Fz+b2Fz+b3Fz
