...
class LowSpeedShaft_drive3pt(ExplicitComponent):
    ''' LowSpeedShaft class
    ...
    def setup(self):
        # variables
        self.add_input('rotor_bending_moment_x', units='N*m', desc='The bending moment about the x axis')
        self.add_input('rotor_bending_moment_y', units='N*m', desc='The bending moment about the y axis')
        self.add_input('rotor_bending_moment_z', units='N*m', desc='The bending moment about the z axis')
        self.add_input('rotor_force_x', units='N', desc='The force along the x axis applied at hub center')
        self.add_input('rotor_force_y', units='N', desc='The force along the y axis applied at hub center')
        ...
        # outputs
        #self.add_output('design_torque', units='N*m', desc='lss design torque')
        #self.add_output('design_bending_load', units='N', desc='lss design bending load')
    ...
    def compute(self, inputs, outputs):
        ...
        rotor_bending_moment_x = inputs['rotor_bending_moment_x']
        rotor_bending_moment_y = inputs['rotor_bending_moment_y']
        rotor_bending_moment_z = inputs['rotor_bending_moment_z']
        rotor_force_x = inputs['rotor_force_x']
        rotor_force_y = inputs['rotor_force_y']
        ...
        mb1Type = 'SRB'
        mb2Type = 'TRB2'
        #mb1Type = inputs['mb1Type']
        #mb2Type = inputs['mb2Type']
        #check_fatigue = inputs['check_fatigue']
        #IEC_Class = inputs['IEC_Class']

        #input parameters
        if flange_length == 0:
            flange_length = 0.3*(rotor_diameter/100.0)**2.0 - 0.1 * (rotor_diameter / 100.0) + 0.4

        if L_rb == 0: #distance from hub center to main bearing
            L_rb = get_L_rb(rotor_diameter, False)[0]
        else:
            L_rb = L_rb

        #If user does not know important moments, crude approx
        if rotor_mass > 0 and rotor_bending_moment_y == 0: 
            rotor_bending_moment_y=get_My(rotor_mass,L_rb)

        if rotor_mass > 0 and rotor_bending_moment_z == 0:
            rotor_bending_moment_z=get_Mz(rotor_mass,L_rb)
