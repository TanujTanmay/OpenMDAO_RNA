...
class RNAWrapper(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # outputs  
        outputs['design_tsr'] = inputs['in_design_tsr'] * 7.0
        outputs['blade_number'] = 3
        outputs['rotor_diameter'] = rotor_diameter
        outputs['hub_radius'] = 1.5 * s
        outputs['root_chord'] = 3.562 * s
        outputs['chord_coefficients'] = [x*y for x,y in zip(inputs['in_chord_coefficients'], [ 3.54, 3.04, 2.26])] 
        outputs['twist_coefficients'] = [x*y for x,y in zip(inputs['in_twist_coefficients'], [13.24, 8.60, 3.18])]
        ...
        print 'TSR = ' + beautify(outputs['design_tsr'])
        print 'TF = ' + beautify(outputs['thickness_factor'])
        print 'chord = ' + beautify(outputs['chord_coefficients'])
        print 'twist = ' + beautify(outputs['twist_coefficients'])
        #print '-'*10
