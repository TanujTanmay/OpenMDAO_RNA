...
import fixed_parameters as params
...
class RotorAero(ExplicitComponent):
    def setup(self):
        ...
        self.add_input('WndSpeed', units = 'm/s', desc = 'wind speed')
        self.add_input('ShearExp', desc = 'wind shear')
        ...
        # outputs
        self.add_output('RtAeroPwr', units='W', desc='rotor power')
        self.add_output('RtAeroCp', desc='rotor Cp')
        self.add_output('RtAeroCq', desc='rotor Cq')
        self.add_output('RtAeroCt', desc='rotor Ct')
        self.add_output('RtTSR', desc='rotor tip speed ratio')
        self.add_output('RtArea', units='m**2', desc='rotor swept area')
        
        for x in ['x', 'y', 'z']:
            self.add_output('RtAeroF' + x + 'h', units='N', desc='Total rotor aerodynamic force in the hub coordinate system')
            self.add_output('RtAeroM' + x + 'h', units='N*m', desc='Total rotor aerodynamic moment in the hub coordinate system')
...
#          
        ...
        [RtAeroPwr, RtAeroCp, RtAeroCq, RtAeroCt, RtTSR, RtArea, \
         RtAeroFxh, RtAeroFyh, RtAeroFzh, RtAeroMxh, RtAeroMyh, RtAeroMzh, \
         TimeSeries] = read_output(output_file, BlSpn)  
#         SpanwiseCl, SpanwiseCd, SpanwiseAlpha, SpanwisePhi, \
#         SpanwiseAxInd, SpanwiseTnInd]    
        
        outputs['RtAeroPwr'] = RtAeroPwr
        outputs['RtAeroCp'] = RtAeroCp
        outputs['RtAeroCq'] = RtAeroCq
        outputs['RtAeroCt'] = RtAeroCt
        outputs['RtTSR'] = RtTSR
        outputs['RtArea'] = RtArea
        outputs['RtAeroFxh'] = RtAeroFxh
        outputs['RtAeroFyh'] = RtAeroFyh
        outputs['RtAeroFzh'] = RtAeroFzh
        outputs['RtAeroMxh'] = RtAeroMxh
        outputs['RtAeroMyh'] = RtAeroMyh
        outputs['RtAeroMzh'] = RtAeroMzh
...
class RotorCalculation(ExplicitComponent):
    def setup(self):
        # variables
        self.add_input('RtAeroCq', desc='rotor Cq')
        self.add_input('RtAeroCt', desc='rotor Ct')
        self.add_input('RtTSR', desc = 'tip speed ratio')
        self.add_input('RtArea', units = 'm**2', desc = 'rotor swept area')
    ...
    def compute(self, inputs, outputs):
        ...
        RtAeroCq = inputs['RtAeroCq']
        RtAeroCt = inputs['RtAeroCt']
        RtTSR = inputs['RtTSR']
        RtArea = inputs['RtArea']
        rotor_diameter = inputs['rotor_diameter']
        WndSpeed = inputs['WndSpeed']
        
        outputs['rotor_torque'] = RtAeroCq * 0.5 * rho * RtArea * (WndSpeed**2.0) * (rotor_diameter/2.0)
        outputs['rotor_thrust'] = RtAeroCt * 0.5 * rho * RtArea * (WndSpeed**2.0)
...
class RotorAeroAdder(Group):
    def setup(self):
        ...
        self.add_subsystem('calc', RotorCalculation(), \
                           promotes_inputs = ['rotor_diameter', 'WndSpeed'], \
                           promotes_outputs=['rotor_torque', 'rotor_thrust'])
        
        self.connect('RtAeroCq', 'calc.RtAeroCq')     
        self.connect('RtAeroCt', 'calc.RtAeroCt')   
        self.connect('RtTSR', 'calc.RtTSR')   
        self.connect('RtArea', 'calc.RtArea')   
...
if __name__ == "__main__":
    ...
    print "My Rotor"
    print prob['aero.RtAeroPwr'] 
    print prob['aero.RtAeroCp'] 
    print prob['aero.RtArea']
    print prob['aero.rotor_torque'] 
    print prob['aero.rotor_thrust'] 
    print prob['aero.RtAeroFxh'], prob['aero.RtAeroFyh'], prob['aero.RtAeroFzh']
    print prob['aero.RtAeroMxh'], prob['aero.RtAeroMyh'], prob['aero.RtAeroMzh']
