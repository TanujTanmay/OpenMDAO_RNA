...
#from PyDSTool.Toolbox.dssrt import GAP
...
def set_primary_input(root_name, BlAFID):
    ...
    if num_nodes <= 9:
        ...
        gap = int(num_nodes)/int(NBlOuts)  
        BlOutNd = ','.join([str(x) for x in range(1, 4*gap + 1, gap)]) + ',' + \
                            str(int(num_nodes)/2 + 1) + ',' + \
                            ','.join([str(x) for x in range(num_nodes - 3*gap, num_nodes + 1, gap)])          
...
def execute_aerodyn(NumBlades, HubRad, HubHt, Overhang, ShftTilt, Precone, \
    ...
    file_name = root_name + str(int(time()))
    driver_file  = file_name + driver_ext
    summary_file = aerodyn_folder + file_name + summary_ext
    output_file  = aerodyn_folder + file_name + output_ext
    
    
    # generate AeroDyn input files
    set_input_driver(file_name, NumBlades, HubRad, HubHt, Overhang, ShftTilt, Precone, \
                     WndSpeed, ShearExp, RotSpd, Pitch, Yaw)          
    set_primary_input(file_name, BlAFID)     
    set_blade_file(file_name, BlSpn, BlTwist, BlChord, BlAFID)
